 ...
 * </ul>
 */
public class S3LogStore extends HadoopFileSystemLogStore {
    ...
    public void write(
        ...
        try {
            if (fs instanceof S3AFileSystem) {
                S3AFileSystem s3AFileSystem = (S3AFileSystem) fs;
                try {

                    // Get the Class object for the class
                    Class<?> clazz = s3AFileSystem.getClass();

                    // Get the private method by name
                    Method privateMethod = clazz.getDeclaredMethod("getAmazonS3Client");

                    // Make the private method accessible
                    privateMethod.setAccessible(true);

                    // Invoke the private method
                    AmazonS3 s3Client = (AmazonS3) privateMethod.invoke(s3AFileSystem);
                    ObjectMetadata metadata = new ObjectMetadata();
                    metadata.setContentType("text/plain");
                    if (!overwrite) {
                        metadata.setHeader("If-None-Match", "*");
                    }
                    // metadata.setContentLength(actions);

                    // Upload the string as an S3 object
                    String key = s3AFileSystem.pathToKey(resolvedPath);
                    StringIteratorInputStream inputStream = new StringIteratorInputStream(actions);
    ...
    public class StringIteratorInputStream extends InputStream {
        ...
        private byte[] currentStringBytes;
        private int currentIndex;

        public StringIteratorInputStream(Iterator<String> iterator) {
            this.iterator = iterator;
            this.currentStringBytes = new byte[0];
            this.currentIndex = 0;
        }

        @Override
        public int read() throws IOException {
            if (currentIndex >= currentStringBytes.length) {
                if (!iterator.hasNext()) {
                    return -1; // End of stream
                }
                String nextString = iterator.next() + "\n"; // Add newline between strings
                currentStringBytes = nextString.getBytes(StandardCharsets.UTF_8);
                currentIndex = 0;
            }
            return currentStringBytes[currentIndex++] & 0xFF;
        }
    }
}
