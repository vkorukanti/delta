...
public class TransactionImpl implements Transaction {
    ...
    try {
      ...
      boolean isFsToCcCommit =
          !readSnapshot.getTableCommitCoordinatorClientHandlerOpt(engine).isPresent()
              && CoordinatedCommitsUtils.getCommitCoordinatorClientHandler(
      ...
      do {
        ...
        try {
          ...
          if (!readSnapshot.getTableCommitCoordinatorClientHandlerOpt(engine).isPresent()) {
  ...
  private TransactionCommitResult doCommit(
    ...
    try (CloseableIterator<Row> stageDataIter = dataActions.iterator()) {
      ...
      if (!wrapEngineExceptionThrowsIO(
          () -> engine.getFileSystemClient().mkdirs(FileNames.commitDirPath(logPath).toString()),
          "Creating directories for path %s",
          logPath)) {
        throw new RuntimeException("Failed to create delta log directory: " + logPath);
      }
  ...
  private void updateMetadataWithCoordinatedCommitsConfs(Engine engine) {
    ...
    if (newCoordinatedCommitsTableConfOpt.isPresent()) {
      Map<String, String> newCoordinatedCommitsTableConf = newCoordinatedCommitsTableConfOpt.get();
      ...
      Map<String, String> coordinatedCommitsTableConfProperty =
          new HashMap<>(metadata.getConfiguration());
      coordinatedCommitsTableConfProperty.put(extraKVConfKey, coordinatedCommitsTableConfJson);
  ...
  private void validateCoordinatedCommitsConfInMetadata() {
    ...
    String newCoordinatedCommitsConf =
        metadata.getConfiguration().get(TableConfig.COORDINATED_COMMITS_COORDINATOR_CONF.getKey());
    String oldCoordinatedCommitsConf =
        readSnapshot
            .getMetadata()
            .getConfiguration()
  ...
  /**
   * This method registers the table with the commit-coordinator via the {@link
   * io.delta.kernel.engine.CommitCoordinatorClientHandler} if the table is transitioning from
   * file-system based table to coordinated-commits table.
   *
   * @param finalMetadata the effective {@link Metadata} of the table. Note that this refers to the
   *     new metadata if this commit is updating the table Metadata.
   * @param finalProtocol the effective {@link Protocol} of the table. Note that this refers to the
   ...
   *     file-system based table to coordinated-commits table. Otherwise, None. This metadata should
   *     be added to the Metadata.configuration before doing the commit.
   */
  protected Optional<Map<String, String>> registerTableForCoordinatedCommitsIfNeeded(
    ...
    Tuple2<Optional<String>, Map<String, String>> oldCommitCoordinatorInfo =
        CoordinatedCommitsUtils.getCoordinatedCommitsConfs(engine, readSnapshot.getMetadata());
    ...
    if (!finalMetadata.getConfiguration().equals(readSnapshot.getMetadata().getConfiguration())
      ...
      Optional<TableCommitCoordinatorClientHandler> oldTableCommitCoordinatorClientHandlerOpt =
          readSnapshot.getTableCommitCoordinatorClientHandlerOpt(engine);
      ...
      if (newCommitCoordinatorClientHandlerOpt.isPresent()
        ...
        logger.info(
            "Table "
                ...
                + "file-system based table to coordinated-commits table: "
                + "[commit-coordinator: "
                + newCommitCoordinatorName.get()
                + ", conf: "
                + newCommitCoordinatorConf
                + "]");
        newCoordinatedCommitsTableConf =
            Optional.of(
                newCommitCoordinatorClientHandlerOpt
                    .get()
                    .registerTable(
                        logPath.toString(),
        ...
        logger.info(
            "Table "
                ...
                + "[commit-coordinator: "
                + oldCommitCoordinatorName.get()
                + ", conf: "
                + oldCommitCoordinatorConf
                + "]");
      } else if (newCommitCoordinatorClientHandlerOpt.isPresent()
          && oldTableCommitCoordinatorClientHandlerOpt.isPresent()
        ...
        String message =
            "Transition of table "
                + logPath
                + " from one commit-coordinator"
                + " to another commit-coordinator is not allowed: [old commit-coordinator: "
                + oldCommitCoordinatorName.get()
                + ", new commit-coordinator: "
                + newCommitCoordinatorName.get()
                + ", old commit-coordinator conf: "
                + oldCommitCoordinatorConf
                + ", new commit-coordinator conf: "
                + newCommitCoordinatorConf
                + "].";
