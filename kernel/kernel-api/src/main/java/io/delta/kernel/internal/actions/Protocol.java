...
public class Protocol {
  ...
  public Set<TableFeature> getImplicitlyEnabledFeatures() {
    if (supportsReaderFeatures() && supportsWriterFeatures()) {
      ...
      return TableFeatures.TABLE_FEATURES.stream()
          ...
          .filter(f -> !supportsReaderFeatures() && f.minWriterVersion() <= minWriterVersion)
   ...
   * from `[appendOnly, changeDataFeed]` to `[appendOnly]` => not allowed. - from `[appendOnly]` to
   * `[appendOnly, changeDataFeed]` => allowed.
   */
  ...
  private boolean supportsReaderFeatures() {
    return TableFeatures.supportsReaderFeatures(minReaderVersion);
  }
