...
class ColumnMappingSuite extends AnyFunSuite {
  ...
  test("column mapping mode change is allowed") {
    ...
    assertThatNoException.isThrownBy(() =>
      ColumnMapping.verifyColumnMappingChange(
        new util.HashMap(),
        Maps.newHashMap(COLUMN_MAPPING_MODE_KEY, NAME.getValue),
        isNewTable))

    assertThatNoException.isThrownBy(() =>
      ColumnMapping.verifyColumnMappingChange(
        Maps.newHashMap(COLUMN_MAPPING_MODE_KEY, NONE.getValue),
        Maps.newHashMap(COLUMN_MAPPING_MODE_KEY, NAME.getValue),
        isNewTable))

    assertThatNoException.isThrownBy(() =>
      ColumnMapping.verifyColumnMappingChange(
        Maps.newHashMap(COLUMN_MAPPING_MODE_KEY, ID.getValue),
        Maps.newHashMap(COLUMN_MAPPING_MODE_KEY, ID.getValue),
        isNewTable))
  ...
  test("column mapping mode change not allowed on existing table") {
    ...
    assertThatThrownBy(() =>
      ColumnMapping.verifyColumnMappingChange(
        Maps.newHashMap(COLUMN_MAPPING_MODE_KEY, NAME.getValue),
        Maps.newHashMap(COLUMN_MAPPING_MODE_KEY, ID.getValue), isNewTable))
    ...
    assertThatThrownBy(() =>
      ColumnMapping.verifyColumnMappingChange(
        Maps.newHashMap(COLUMN_MAPPING_MODE_KEY, ID.getValue),
        Maps.newHashMap(COLUMN_MAPPING_MODE_KEY, NAME.getValue), isNewTable))
      .isInstanceOf(classOf[IllegalArgumentException])
